...
module.exports = function(options){
	...
	function NodeClam(options) {
		...
		// 		console.log("node-clam: ClamAV could not be found at " + this.settings.clam_path + "!");
		// }
	...
	NodeClam.prototype.is_infected = function(file,callback) {
		...
		exec(command, function(err, stdout, stderr) { 
			if(stderr || stdout.indexOf('ERROR: ') == 0) {
				if(self.settings.debug_mode)
					console.log(stderr);
				if (stderr) {
					callback(stderr, file, null);
				} else if (stdout.indexOf('ERROR: ') == 0) {
					callback(stdout, file, null);
				};
				...
				if (result.match(/: .+(?= FOUND)/)) {
					var virusName = result.match(/: .+(?= FOUND)/)[0].substring(2);
				};
				if(result.match(/OK$/)) {
					if(self.settings.debug_mode)
					...
					try{						
						if (virusName) {
							callback(null, file, true, virusName);
						} else {
